...
class BaseRepository {
    constructor(model = null) {
        this.get = (documentId, attributes = { password: 0 }) => __awaiter(this, void 0, void 0, function* () {
            console.log('base.repository ', documentId);
            return yield this.model.findById(documentId).select(attributes);
        });
        this.getAll = (query = {}, sort = {}, pageSize = 5, pageNum = 1, attributes = { 'password': 0 }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (resultTotalSize > 0) {
                ...
                result = yield this.model.find(query).sort(sort).skip(skips).limit(pageSize).select(select[0]).select(select[1]);
            }
